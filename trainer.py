...
class Trainer():
    def __init__(self, model:TrainerClass, cfg):
        ...
        assert(not(cfg["step_scheduler"]==True and cfg["validation_scheduler"]==True))
    ...
    def __write_to_tensorboard(self, metrics_data):
        ...
        for _, (key, val) in enumerate(metrics_data):
            ...
            if key == "loss":
                continue
            writer.add_scalar('Metric/{}_{}' .format(key, self.description), self.metric_container[key].avg, self.global_step)
            writer.add_scalar('Metric/{}_all_runs' .format(key), self.metric_container[key].avg, self.global_step)


        # writer to tensorboard
        writer.add_scalar('Loss/train_{}' .format(self.description), 
                          self.metric_container["loss"].avg, self.global_step)
        ...
        for idx, lr_value in enumerate(self.__get_learning_rate()):
            writer.add_scalar("Learning_rates/lr_{}_{}" .format(idx, self.description), lr_value, self.global_step)
    ...
    def __train_one_epoch(self, dataloader):
        ...
        for step in pbar:
            ...
            pbar.set_description(f"TRAIN epoch {self.current_epoch+1}/{epochs} idx {step} current loss {current_loss}, avg loss {avg_loss}")
            ...
            new_row = [self.current_epoch, step, loss.detach().item()] +  [metric.avg for metric in self.metric_container.values()] + [self.__get_learning_rate(), saving_best]
    ...
    def __validation_step(self, dataloader, validation_metric):
        ...
        with torch.no_grad():
            ...
            for step in pbar:
                ...
                #set pbar description
                pbar.set_description(f"VALIDATION epoch {self.current_epoch+1}/{epochs} step {step} current loss {validation_sum_loss.current}, avg loss {validation_sum_loss.avg}, validation_metric {val_metric.avg}")
            ...
            logging.info("Validation result metric for epoch {} = {}" .format(self.current_epoch, val_metric.avg))
                
            if self.best_validation_metric > val_metric.avg:
                file_name = "best_model_validation_{}.pth" .format(self.description)
